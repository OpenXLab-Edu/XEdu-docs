...
这个方法将会在指定的主机地址（`localhost:7860`）上启动一个 Web 服务器，并在浏览器中提供一个用户界面，方便用户进行实时对话。

##### 参数说明

- `host` (str): 指定 Web 服务器的主机地址。默认值为 `'0.0.0.0'`，表示本地回环地址，仅能本机访问。你也可以将其设置为其他 IP ，如`'127.0.0.1'`，使其可在局域网中访问。
...
2. **本地主机访问**： 目前仅允许本机访问。如果希望其他设备也能访问，需要进行以下修改：
   ...
   - **修改方式2**：将 `host` 设置为 `'127.0.0.1'`，表示开放所有外部 IP 访问。例如：
