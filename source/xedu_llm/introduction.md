...
这个方法将会本机上启动一个 Web 服务，并提供一个用户界面，方便用户进行实时对话。默认服务地址为 http://127.0.0.1:7860，可以通过本机浏览器访问，如果想通过其他局域网设备访问，可以通过访问本机局域网IP地址+端口号（如http://192.168.1.123:7860）访问。
...
- **仅允许本机访问**：127.0.0.1表示本地回环地址，仅能本机访问。

  ```python
  chatbot.run(host='127.0.0.1')
  ```
  
- **允许特定局域网内设备访问**：如果希望其他设备也能访问，可将 `host` 做修改，例如 `'0.0.0.1'`，表示开放所有外部 IP 访问。例如：

  ```python
  chatbot.run(host='0.0.0.1', port=7860)
  ```
