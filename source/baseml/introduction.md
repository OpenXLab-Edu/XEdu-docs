...
**方法1：**使用`load_tab_data`方法直接载入一个CSV文件。对CSV文件有严格格式要求：数据文件每行一条记录，输入数据（特征）列在前，输出数据（目标或标签）列在后，即最后一列为输出数据，其余列为输入数据，且要求数据类型为数值类型并无缺失值。此方式默认载入数据集的全部特征。
...
**方法2：**使用`load_dataset`方法载入，需指定文件类型、特征列和标签列，可辅助做特征选择，这种方法载入数据更加灵活。
